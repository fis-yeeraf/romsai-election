<template>
  <div class="flex gap-2">
    ...
    <div v-if="totalVotes > 0" class="flex flex-1 rounded-full overflow-hidden">
      <div
        class="text-center text-white font-bold text-xs py-1 transition duration-300"
        ...
        :key="candidate.id"
        :style="{ width: `${((candidate.total_votes || 50) / (totalVotes || 100)) * 100}%` }"
      >
        {{ candidate.total_votes?.toLocaleString() || 0 }}
      </div>
    </div>
    <div v-else class="flex flex-1 rounded-full overflow-hidden">
      <div class="text-center text-white font-bold text-xs py-1 bg-orange-500 w-1/2">0</div>
      <div class="text-center text-white font-bold text-xs py-1 bg-blue-500 w-1/2">0</div>
    </div>
  </div>
</template>
